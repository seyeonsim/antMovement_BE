...
const getOriginNews = async (originUrl) => {
    try {
        ...
        $('#dic_area > span.end_photo_org > em').remove();

        let subTitle = $('#dic_area > strong').first().text();
        $('#dic_area > strong').remove();
        ...
        $('#dic_area > div').first().remove();

        let newContent = $('#dic_area')
            .html()
            .replace(/\s{2,}/g, '');
        newContent = newContent.replace(/<br\s*\/?>/g, '\n');

        let modifiedContent = $('<div>').html(newContent);
        modifiedContent = modifiedContent.text().trim();
    ...
    try {
        ...
        for (const dataList of listArray) {
            ...
            try {
                ...
                const content = bigImageAndContent.modifiedContent;
